    ...
    <div className="bg-white border border-zinc-200/90 rounded-2xl p-5 sm:p-6 shadow-xs space-y-5">
      ...
      <div>
        <div className="flex items-center justify-between">
          <label htmlFor="gross-income-input" className="text-xs font-bold text-zinc-900 flex items-center gap-1.5">
        ...
        <div className="mt-2 relative flex items-center border border-zinc-300 focus-within:border-emerald-700 focus-within:ring-2 focus-within:ring-emerald-700/15 rounded-xl px-3.5 py-2.5 bg-white transition shadow-2xs">
          ...
          <input
            ...
            className="w-full text-xl sm:text-2xl font-bold tracking-tight text-zinc-900 outline-none bg-transparent tabular-nums placeholder:text-zinc-300"
          />
          {grossDigits && (
            <button
              type="button"
              onClick={() => setGrossDigits("")}
              className="p-1 text-zinc-400 hover:text-zinc-600 rounded-md transition cursor-pointer shrink-0"
              title="Clear input"
            >
              <X className="w-4 h-4" />
            </button>
        ...
        <div className="mt-2 flex items-center justify-between text-[11px] text-zinc-500">
          ...
          {grossAnnual <= 250000 && grossAnnual > 0 && (
            <span className="text-emerald-700 font-semibold">{t.taxExemptBadge}</span>
          )}
        </div>
        ...
        <div className="mt-2 text-[10.5px] text-zinc-400 font-medium">
          {t.autoUpdateNote}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 p-0.5 bg-zinc-100 rounded-xl gap-1 border border-zinc-200/70">
          <button
            type="button"
            onClick={() => setPeriod("annual")}
            className={`py-1.5 px-3 text-xs font-semibold rounded-lg transition cursor-pointer ${
              period === "annual" ? "bg-white text-zinc-900 shadow-2xs font-bold" : "text-zinc-500 hover:text-zinc-800"
            }`}
          >
            {t.periodAnnual}
          </button>
          <button
            ...
            className={`py-1.5 px-3 text-xs font-semibold rounded-lg transition cursor-pointer ${
              period === "quarterly" ? "bg-white text-zinc-900 shadow-2xs font-bold" : "text-zinc-500 hover:text-zinc-800"
            }`}
          >
            {t.periodQuarterly}
          </button>
        ...
        <div className="grid grid-cols-2 p-0.5 bg-zinc-100 rounded-xl gap-1 border border-zinc-200/70">
          <button
            ...
            className={`py-2 px-3 text-xs font-semibold rounded-lg transition flex items-center justify-center gap-1.5 cursor-pointer ${
              !isMixed ? "bg-white text-zinc-900 shadow-2xs font-bold" : "text-zinc-500 hover:text-zinc-800"
            }`}
          >
            {t.pureFreelancer}
          </button>
      ...
      {isMixed && (
        <div className="p-3.5 bg-zinc-50 border border-zinc-200 rounded-xl space-y-2">
          ...
          <div className="flex items-center border border-zinc-300 rounded-lg px-3 py-2 bg-white focus-within:ring-2 focus-within:ring-emerald-700/15">
      ...
      {isOver3M && (
        <div className="p-3 bg-amber-50 border border-amber-300/80 rounded-xl flex items-start gap-2.5 text-amber-900">
      ...
      <div className="border-t border-zinc-100 pt-3">
        <button
          ...
          className="w-full flex items-center justify-between px-3 py-2 text-xs font-semibold text-zinc-700 hover:text-zinc-900 bg-zinc-50 hover:bg-zinc-100/80 rounded-xl border border-zinc-200/80 transition cursor-pointer"
        >
          <div className="flex items-center gap-2">
            <span>{showAdvanced ? "−" : "+"}</span>
            <span>{t.advancedOptionsToggle}</span>
            {hasActiveAdvanced && (
              <span className="w-2 h-2 rounded-full bg-emerald-600" title="Active settings" />
            )}
          </div>
          <span className="text-[10.5px] text-zinc-400 font-normal hidden sm:inline">
          ...
          <div className="mt-3 space-y-4 pt-1">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-semibold bg-emerald-50 text-emerald-800 px-2 py-0.5 rounded border border-emerald-200/50">
                  {t.deductibleBadge}
                </span>
              </div>
              <p className="text-[11px] text-zinc-500 leading-normal">
              ...
              {cwtMode === "custom" && (
                <div className="flex items-center border border-zinc-300 rounded-lg px-3 py-2 bg-white focus-within:ring-2 focus-within:ring-zinc-900/10">
                  <span className="text-zinc-400 font-serif text-sm mr-2">₱</span>
                  <input
                    type="text"
                    inputMode="numeric"
                    ...
                    className="w-full text-xs font-bold text-zinc-900 outline-none bg-transparent tabular-nums placeholder:text-zinc-300"
                  />
                </div>
              )}
            </div>
            ...
            <div className="space-y-2 border-t border-zinc-100 pt-3">
              {!showExpenses ? (
                <button
                  ...
                  className="w-full py-2 px-3 border border-dashed border-zinc-300 rounded-xl text-xs font-medium text-zinc-600 hover:text-zinc-900 hover:border-zinc-400 bg-white transition flex items-center justify-center gap-1.5 cursor-pointer"
                >
