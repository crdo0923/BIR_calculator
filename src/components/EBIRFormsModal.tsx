      ...
      >
        ...
        <div className="px-5 sm:px-6 py-4 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between bg-white/95 dark:bg-zinc-900/95 backdrop-blur-md sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 id="ebir-modal-title" className="text-base sm:text-lg font-bold text-zinc-900 dark:text-zinc-100 tracking-tight">
        ...
        <div className="px-5 sm:px-6 py-4 overflow-y-auto space-y-4">
          {/* Instructions Box */}
          <div className="p-4 bg-emerald-50/80 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-800/60 rounded-2xl text-xs text-emerald-950 dark:text-emerald-200 flex items-start gap-3 shadow-2xs">
            <div className="w-7 h-7 rounded-xl bg-emerald-600 text-white flex items-center justify-center shrink-0 mt-0.5">
              <ShieldCheck className="w-4 h-4" />
            </div>
            <div className="space-y-1">
              <div className="font-bold text-emerald-900 dark:text-emerald-300 text-xs sm:text-sm">
          ...
          {/* Line Items Table */}
          <div className="border border-zinc-200/90 dark:border-zinc-800 rounded-2xl overflow-hidden divide-y divide-zinc-100 dark:divide-zinc-800 shadow-2xs bg-white dark:bg-zinc-900">
            {lines.map((item) => (
              <div
                key={item.num}
                className={`p-3.5 sm:p-4 flex items-center justify-between gap-3 transition ${
                  item.highlight ? "bg-emerald-50/70 dark:bg-emerald-950/40 border-l-4 border-l-emerald-600 dark:border-l-emerald-400" : "hover:bg-zinc-50/80 dark:hover:bg-zinc-800/50"
                }`}
              >
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-zinc-900 dark:text-zinc-100 text-xs sm:text-sm truncate">{item.label}</span>
                  </div>
                  <div className="text-[11px] text-zinc-500 dark:text-zinc-400 mt-1 pl-1">{item.note}</div>
                </div>

                <div className="flex items-center gap-2.5 shrink-0">
                  <span
                    className={`font-mono text-sm sm:text-base tabular-nums font-bold ${
                      item.highlight ? "text-emerald-700 dark:text-emerald-400 text-base sm:text-lg font-black" : "text-zinc-900 dark:text-zinc-100"
                    }`}
                  >
                    {item.value}
                  </span>
                  <button
                    ...
                    className={`p-2 rounded-xl border transition cursor-pointer active:scale-95 flex items-center justify-center ${
                      copiedKey === item.num
                        ...
                        : "bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 shadow-2xs"
                    }`}
                    title={isEn ? "Copy value" : "Kopyahin ang halaga"}
                  >
                    {copiedKey === item.num ? (
                      <Check className="w-4 h-4 text-emerald-600 dark:text-emerald-400 animate-in zoom-in duration-150" />
                    ) : (
          ...
          <div className="p-4 bg-zinc-50/80 dark:bg-zinc-950/60 rounded-2xl border border-zinc-200/80 dark:border-zinc-800 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-zinc-500 dark:text-zinc-400 font-medium">Local Fast Offline Assets</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              <a
                href="/forms/Offline-eBIRForms-Package-v7.9.4.2.zip"
                download="Offline-eBIRForms-Package-v7.9.4.2.zip"
                className="flex items-center gap-2.5 p-2.5 bg-white dark:bg-zinc-900 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-900 dark:text-zinc-100 rounded-xl text-xs font-semibold border border-zinc-200 dark:border-zinc-800 shadow-2xs transition cursor-pointer group"
              >
                <div className="w-8 h-8 rounded-lg bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200/60 dark:border-emerald-800/60 flex items-center justify-center shrink-0 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-900/60 transition">
                  <Download className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="truncate font-bold">eBIRForms Package</div>
                  <div className="text-[10px] text-zinc-500 dark:text-zinc-400">v7.9.4.2 (.zip)</div>
                </div>
              </a>
              ...
              <a
                ...
                className="flex items-center gap-2.5 p-2.5 bg-white dark:bg-zinc-900 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-900 dark:text-zinc-100 rounded-xl text-xs font-semibold border border-zinc-200 dark:border-zinc-800 shadow-2xs transition cursor-pointer group"
              >
                <div className="w-8 h-8 rounded-lg bg-sky-50 dark:bg-sky-950/60 border border-sky-200/60 dark:border-sky-800/60 flex items-center justify-center shrink-0 group-hover:bg-sky-100 dark:group-hover:bg-sky-900/60 transition">
                  <FileDown className="w-4 h-4 text-sky-600 dark:text-sky-400" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="truncate font-bold">{formName}</div>
                  <div className="text-[10px] text-zinc-500 dark:text-zinc-400">Official BIR Form (PDF)</div>
                </div>
              </a>
            </div>

            {/* Official BIR Online Portal Link */}
            <div className="pt-2 border-t border-zinc-200/70 dark:border-zinc-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1 text-[11px] text-zinc-500 dark:text-zinc-400">
              <span>
                {isEn
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 font-bold text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 dark:hover:text-emerald-300 hover:underline shrink-0"
              >
          ...
          <div className="text-xs text-zinc-600 dark:text-zinc-400 space-y-2.5 bg-zinc-50/80 dark:bg-zinc-950/60 p-4 rounded-2xl border border-zinc-200/70 dark:border-zinc-800">
            <div className="flex items-center justify-between">
              ...
              {onOpenPaymentGuide && (
                <button
                  ...
                  className="inline-flex items-center gap-1 font-bold text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 dark:hover:text-emerald-300 text-xs hover:underline cursor-pointer"
                >
        ...
        {/* Sticky Footer */}
        <div className="px-5 sm:px-6 py-3.5 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/95 dark:bg-zinc-900/95 backdrop-blur-md flex flex-wrap items-center justify-between gap-2.5 sticky bottom-0 z-10">
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handleDownloadWorksheet}
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold text-zinc-900 dark:text-zinc-100 bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 active:scale-95 border border-zinc-300 dark:border-zinc-700 transition cursor-pointer shadow-2xs"
            >
            ...
            <button
              type="button"
              onClick={handlePrint}
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold text-zinc-700 dark:text-zinc-300 bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 active:scale-95 border border-zinc-200 dark:border-zinc-700 transition cursor-pointer"
            >
